...
# $Header: /mhub4/sources/imap-tools/imapdump.pl,v 1.57 2016/06/28 18:04:42 rick Exp $
...
my %DUMPED;
   ...
   namespace( $conn, \$srcPrefix, \$srcDelim );
   ...
   foreach $mbx ( @mbxs ) {
        ...
        if ( $srcDelim eq '.' ) {
          ...
          $mailbox =~ s/$srcDelim/\//g;
        }

        if ( $opt_W ) {
           #  Just create the directories for the mailboxes, don't dump any msgs
           $mbx =~ s/^$srcPrefix//;
           if ( $srcDelim eq '.' ) {
              $mbx =~ s/\./\//g;
           } else {
           ...
           Log("mbx $mbx");
           Log("Creating $dir/$user/$mbx directory\n");
        ...
        foreach $msgnum ( @msgs ) {
             ...
             Log("$msgnum  --- $msgid flags   --- $flags") if $debug;
             ...
             mkpath( "$dir/$user/$mailbox" ) if !-d "$dir/$user/$mailbox";
             ...
             $msgfile = unique( $msgfile, "$dir/$user/$mailbox" );
             ...
             } elsif ( $sync_flags and $flags =~ /Seen|Deleted/i ) {
                my $FLAGS;
                $FLAGS = 'S' if $flags =~ /Seen/i;
                $FLAGS .= 'D' if $flags =~ /Deleted/i;
                $msgfile .= ",$FLAGS" if $FLAGS;
             }

             if ( !open (M, ">$dir/$user/$mailbox/$msgfile") ) {
                Log("Error opening $dir/$user/$mailbox/$msgfile: $!");
                $dump_errors++;
                next;
             }
             ...
             set_file_date( "$dir/$user/$mailbox/$msgfile", $date ) if $set_file_date;
        ...
        # Remove from the dump any messages no longer on the source

        delete_from_dump( $dir, $user, $mbx, \@source_msgs, $conn ) if $del_from_dump;
   }
...
sub delete_from_dump {

my $dir    = shift;
my $user   = shift;
my $mbx    = shift;
my $source_msgs = shift;
my $conn   = shift;

   #  If -k (del_from dump) is set then remove from the dump any messages
   #  no longer on the source

   my %source_msgs;
   foreach $_ ( @$source_msgs ) {
      # /\|+(.+)$/;
      my @terms = split(/\|/, $_);
      $msgid = $terms[-1];
      $source_msgs{"$msgid"} = 1;
      Log("source $msgid") if $debug;
   }

   #  Get list of msgids from the messages in the dump

   my $mbx_dir = "$dir/$user/$mbx";
   my %dump_msgids;
   get_msgids_from_dump_file( $mbx_dir, \%dump_msgids );

   Log("See if any dumped messages no longer exist on the source");
   $n = keys %dump_msgids;

   $deleted=0;
   foreach $dump_msgid ( keys %dump_msgids ) {
      $fn = $dump_msgids{"$dump_msgid"};
      Log("checking $fn   $dump_msgid") if $debug;
      if ( $source_msgs{"$dump_msgid"} eq '' ) {
         #  The msg in the dump no longer exists on the source.  Delete it.
         foreach $file ( split(/\s+/, $fn ) ) {
            Log("Msgid $dump_msgid no longer exists on the source. Deleting $file");
            unlink $file;
            $deleted++;
         }
      }
   }
   Log("Deleted $deleted msgs from $mbx dump");

}

sub get_msgids_from_dump_file {

my $dir = shift;
my $msgids = shift;

   #  Get list of msgids in the dump for this mailbox

   my @dump_msgs;

Log("Getting msgids from the dump");
   %$msgids = ();
   opendir D, $dir;
   my @files = readdir( D );
   closedir D;
   foreach $_ ( @files ) {
      next if /^\./;
      my $fn = "$dir/$_";
      $msgid = get_msgid_from_dumpfile( $fn );
      if ( $msgid ) {
         $$msgids{"$msgid"} .= "$fn ";
      }
   }
...
sub get_msgid_from_dumpfile {

my $fn = shift;
my ($msgid,$subject,$date,$from);

   #  Extract the msgid from the dump message file
   ...
   if ( $msgid eq '' ) {
      $msgid = "$date$subject$from";
      $msgid =~ s/\s+//g;
      $msgid =~ s/\+|\<|\>|\?|\*|"|'|\(|\)|\@|:|-|,|\.|\[|\]//g;
      Log("Msgid missing, built one as $msgid");
   }
...
sub init {
   ...
   if ( $del_from_dump ) {
      Log("Msgs no longer on the source will be deleted from the dump");
   }
...
sub usage {
   ...
   print STDOUT "          -r remove messages from the server after dumping them\n";
   ...
   print STDOUT "          [-k] Delete from the dump messages no longer on the source \n";
...
sub processArgs {
   ...
   if ( !getopts( "dS:L:m:hf:F:Ix:ra:uD:Ue:E:A:sgR:l:n:GwzCX:YZ:Ni:j:T:qcpkW" ) ) {
      usage();
   }
...
my @numbers;
   ...
   if ( $extension ) {
      #  Add the extension unless it already has one
      $fn .= $extension if $fn !~ /$extension$/;
   }
