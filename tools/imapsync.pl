...
# $Header: /mhub4/sources/imap-tools/imapsync.pl,v 1.83 2016/06/17 22:23:28 rick Exp $
   ...
   for $i (0 .. $#response) {
	last if $response[$i] =~ /^1 OK FETCH complete/i;
        ...
        if ( $response[$i] =~ /^\)/ or ( $response[$i] =~ /\)\)$/ ) ) {
           ...
           Log("msgnum=$msgnum msgid=$msgid") if $verbose;
           if ( $msgnum eq '' ) {
              Log("msgnum is blank");
              next;
           }
           if ( $msgid eq '' ) {
              ...
              $date1 = convert_date( $header_date );
              $date1 =~ s/\W//g;
              $date1 =~ s/^0//;

              $subject =~ s/\W//g;
              $msgid = "$date1$subject$from";
              $msgid =~ s/\s+//g;
              $msgid =~ s/\+|\<|\>|\?|\*|"|'|\(|\)//g;
	   ...
	   push (@$msgs,"$msgid||||||$msgnum||||||$flags||||||$date||||||$header_date||||||$uid");
           ...
           $msgnum=$msgid=$date=$flags=$header_date=$internaldate=$date1=$from=$subject=$uid='';
        }
   }
}
...
my $uid;
    ...
    #
    #  Get list of messages sent in the specified date range reference dates
    #
    $nums = "";

    my $filter;
    if ( $since and $before ) {
       $filter = "SENTSINCE \"$since\" SENTBEFORE \"$before\"";
    } elsif ( $since ) {
       $filter = "SENTSINCE \"$since\"";
    } elsif ( $before ) {
       $filter = "SENTBEFORE \"$before\"";
    }
    Log("     Get messages $filter") if $debug;
    ...
    if ( $nums eq "" ) {
	Log ("     $mailbox has no messages $filter") if $debug;
	return;
    }
	...
	if   ( $response =~ /^1 OK/i ) {
     ...
     foreach $_ ( @response ) {
          ...
          if ( /^\)/ or ( /\)\)$/ ) ) {
             ...
             $$list{"$msgid"} = "$msgnum,$flags";
             push (@$msgs,"$msgid||||||$msgnum||||||$flags||||||$date||||||$header_date||||||$uid");
             $msgnum=$msgid=$date=$flags=$internaldate=$header_date=$date1=$from=$subject=$uid='';
          }
      }
   }
...
sub usage {
   ...
   print STDOUT "          -b <before> Sync messages before this date (DD-MMM-YYYY) or number of days ago\n";
...
sub processArgs {
   ...
   if ( !getopts( "dvS:D:L:m:e:hIx:y:M:s:nNqu:E:F:f:i:RWtA:YZb:C" ) ) {
      usage();
   }
...
#       next if $nosel_mbxs{"$src_mbx"};
	...
	@sourceMsgs=();
        ...
        if ( $sync_since or $sync_before ) {
           getDatedMsgList( $src_mbx, $sync_since, $sync_before, \@sourceMsgs, \%sourceList, $src );
        } else {
...
my $total_deletes=0;
   ...
   if ( $sync_since ) {
      getDatedMsgList( $dst_mbx, $sync_since, $sync_before, \@destMsgs, \%destList, $dst );
   } else {
      getMsgList( $dst_mbx, \@destMsgs, \%destList, $dst );
   }
   ...
   foreach $_ ( @destMsgs ) {
      ...
      if ( $verbose ) {
         Log("Does $msgid on the dest exist on the src?");
         if ( !$$sourceList{"$msgid"}  ) {
            Log("$msgid does not exist on the source.  need to delete it");
         } else {
            Log("$msgid does exist on the source");
         }
      }
...
my $date1;
   ...
   #  Dates in messages are often poorly formatted.  Try to clean
   #  it up.

   return $date unless $convert_date;

   if ( $date =~ /^(.+)-(.+)-(.+)/ ) {
      # DateTime::Format::Mail does not like this format with
      # dd-mmm-yyyy but will accept dd mmm yyyy.
      $date =~ s/-/ /;
      $date =~ s/-/ /;
   }

   $date1 = eval 'DateTime::Format::Mail->parse_datetime($date)';
   if ( $@ ) {
      Log("Bad date: $date");
      $date1 = $date;
   }
