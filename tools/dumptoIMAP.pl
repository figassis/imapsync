...
foreach $mbx ( @mbxs ) {
   ...
   if ( $update ) {
      $imap_mbx =~ s/\//$mbx_delim/g unless $mbx_delim eq '/';
      if ( $prefix ) {
         $imap_mbx = $prefix . $imap_mbx unless $imap_mbx =~ /^INBOX/i;
      }
      getMsgList( $imap_mbx, \%IMAP_MSGIDS, $conn );

      if ( $debug )  {
         Log("Messages in IMAP mbx $imap_mbx:");
         foreach $msgid ( keys %IMAP_MSGIDS ) {
            Log("imap $msgid");
         }
      }
   }

   #  Get the messages in the dump
   get_messages( "$dir/$mbx", \@msgs, \%DUMP_MSGIDS );
   ...
   foreach $_ ( @msgs ) {
      ...
      if ( $update ) {
         ...
         next if $IMAP_MSGIDS{"$msgid"};   # Msg already exist in IMAP
         Log("Need to copy $msgid");
      }
...
sub init {
   ...
   if ( !getopts('m:L:i:dD:Ix:XRA:F:y:r:u') ) {
      usage();
   }
   ...
   Log("Starting");
   if ( $update ) {
      Log("Running in update mode.  Messages in the dump will be copied only if they do not already exist in IMAP");
   }
...
sub usage {
   ...
   print "    [-u update mode, don't copy message if it already exists in mailbox\n";
   print "    [-L <logfile>]\n";
   print "    [-d debug]\n";
...
my %MSGIDS;
   ...
   foreach $_ ( sort @files ) {
      ...
      if ( $update ) {
         $msgid = getMsgid( $msgfn );
         $$DUMP_MSGS{"$msgfn"} = $msgid;
      }
      push( @$msgs, $msgfn );
   }
...
my %MBXS;
   ...
   $n = scalar @messages;
   Log("There are $n messages in the dump");
...
my $msgid;
   ...
   #  Read the dumped message to get its Message-ID

   if (! open( MSG, "<$fn" ) ) {
      Log("Can't open message $fn: $!");
      return 0;
   }
   while( <MSG> ) {
      chomp;
      last if length( $_ ) == 1;    # End of header
      if ( /^Message-ID:\s*(.+)/i ) {
         $msgid = $1;
         $msgid =~ s/\r$//;
         last;
