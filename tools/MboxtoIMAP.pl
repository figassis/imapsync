...
# $Header: /mhub4/sources/imap-tools/MboxtoIMAP.pl,v 1.25 2016/04/21 11:05:28 rick Exp $
    ...
    foreach $mailfile ( @mailfiles ) {
       ...
       if ( !isAscii( $mbx ) ) {
          ...
          if ( $modified_utf7_installed ) {
             $mbx = encode_mbx_name( $mbx );
          } else {
             Log("The name $mbx contains non-ASCII characters.  To have it properly");
             Log("named in IMAP you must install the Encode::IMAPUTF7 Perl module");
          }
       } 
...
my $imapmbx = shift;
   ...
   #  Encode the mailbox name in UT7 if necessary
