...
# $Header: /mhub4/sources/imap-tools/imapcopy.pl,v 1.177 2016/04/03 14:33:02 rick Exp $
...
my %MESSAGEIDS;
   ...
   if ( $search_filter ) {
      $range = search( $mailbox, $search_filter, $conn );
      $range =~ s/\s+/,/g;
   } else {
      my $start = 1;
      my $end   = '*';
   ...
   sendCommand ( $conn, "1 FETCH $range (uid flags internaldate RFC822.SIZE body.peek[header.fields (From Date Message-Id Subject)])");
...
sub usage {
   ...
   print STDOUT "          -z  Select msgs to copy using an IMAP Search filter.\n";
...
sub processArgs {
   ...
   if ( !getopts( "dS:D:L:m:hIp:M:rqx:y:e:Rt:Tia:b:X:vPA:UB:E:uHZ:j:g:CsnWwGF:cK:Y:kfQJ:lo:OVNz:" ) ) {
      usage();
   }
   if ( $opt_S =~ /\\/ ) {
...
my $loops;
    ...
    #  Use an IMAP SEARCH filter to select the messages to be copied
