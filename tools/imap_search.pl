...
# $Header: /mhub4/sources/imap-tools/imap_search.pl,v 1.10 2016/06/21 13:23:45 rick Exp $
...
use MIME::Base64 qw(decode_base64 encode_base64); 
   ...
   #  Search a user's mailbox using an IMAP SEARCH expression
   ...
   login($sourceUser,$sourcePwd, $sourceHost, $admin_user, $admin_pwd, $src, $srcMethod) or exit;
   ...
   if ( $opt_m ) {
      $where = "in $opt_m";
      $where .= " and subfolders" if $opt_r;
   } else {
      $where = "in all folders\n";
   }
   Log("Searching for '$search_filter' $where");
...
sub init {
   ...
   if ( !getopts( "S:s:IdL:m:ru:" ) ) {
      usage();
   }

   read_config();

   if ( !$sourceHost and !$opt_S ) { 
      usage();
      exit;
   }
   ...
   if ( $opt_S =~ /\\/ ) {
      ($sourceHost, $sourceUser, $sourcePwd,$srcMethod) = split(/\\/, $opt_S) if $opt_S;
   } else {
      ($sourceHost, $sourceUser, $sourcePwd,$srcMethod) = split(/\//, $opt_S) if $opt_S;
   }
   ...
   $search_filter =~ s/\s+$//;
   $search_filter =~ s/\s*""//g;
   $logfile = "imap_search.log" unless $logfile;
   open(LOG, ">>$logfile");

   $sourceUser = $user if $user;
   $os = $ENV{'OS'};
   ...
   unless( $sourceUser and ($sourcePwd or $admin_pwd) and $sourceHost ) {
      usage();
   }
...
sub read_config {
   ...
   foreach $dir ( @dirs ) {
     ...
     if ( -e $cf ) {
        print "Reading config file $dir/imap_search.cf\n";
        open( CF, "<$cf");
        while( <CF> ) {
           chomp;
           s/^\s+//;
        ...
        if ( $KW{server} and $KW{admin_user} and $KW{admin_pwd} ) {
...
my $method = shift;
   ...
   if ( uc( $method ) eq 'CRAM-MD5' ) {
      ...
      my $rc = login_cram_md5( $sourceUser, $sourcePwd, $conn );
      return $rc;
   }

   if ( $admin_user and $admin_pwd ) {
      login_plain( $sourceUser, $admin_user, $admin_pwd, $conn ) or exit;
      return 1;
   }  

   if ( $pwd =~ /^oauth2:(.+)/i ) {
      $token = $1;
      login_xoauth2( $sourceUser, $token, $conn );
      return 1;
   }

   #  Otherwise do an ordinary login

   sendCommand ($conn, "1 LOGIN $sourceUser \"$sourcePwd\"");
   while (1) {
	readResponse ( $conn );
	last if $response =~ /^1 OK/i;
...
my $loops;
    ...
    Log("$nums") if $debug;
    if ( $nums eq "" ) {
	Log ("     $mailbox has no messages matching the filter: $filter") if $debug;
	return;
    }
...
sub usage {
   ...
   print "\nUsage: $0 
          -S host/user/pwd 
          -s <IMAP SEARCH EXPRESSION>
          [-m <folder list>] folder1,folder2,etc.  Default is all folders
          [-r] If list of folders supplied with -m then also get submailboxes under each
          [-L <logfile>] default is imap_search.log
          [-I] print IMAP commands and responses
          [-d] debug mode
   \n";
...
sub validate_search_filter {

my $filter = shift;

   #  Make sure the expression supplied by the user is
   #  valid.

   Log("Validating syntax of search filter $filter");
   if ( $filter =~ /(.+)=(.+)/ ) {
      Log("Filter syntax error: KW=VALUE format is not valid");
      filter_examples();
      exit;
   }
...
sub filter_examples {
   ...
   print "Examples of correct syntax
      from joe@abc.com subject test message 
      AND search1 search2 etc
      OR search1 search2 etc
      NOT search1 search2 etc

      Note that when multiple search terms are used it is not necessary
      to use the 'AND' operator as it is implied.

      See https://tools.ietf.org/html/rfc3501#section-6.4.4 for more information
      on the IMAP SEARCH command including keywords and syntax.
";
}
